...
//		if(isAlreadyCreated(car)){
//			errors.rejectValue("registration", "car.registration.duplicate", "");
//		}
		storeCar(car);
	}

	private void storeCar(Car newCar) {
		
			Car existingCar = carService.findCarByRegistration(newCar.getRegistration());
			if (existingCar == null) {
				carService.saveCar(newCar);
			} else {
				carService.mergeCar(newCar, existingCar);
			} 
